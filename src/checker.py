...
class ProxyChecker:
    ...
    def _requester(self, socks: str):
        proxy = {
            "http": "socks5h://%s" % socks,
            "https": "socks5h://%s" % socks
        }
        for retry in range(RETRIES):
            try:
                response = requests.get(
                    url=URL_FOR_CHECK, headers=self.HEADERS,
    ...
    def pool_requests(self):
        ...
        with Pool(THREADS) as pool:
            for result in tqdm(pool.imap_unordered(self._requester, self.socks_list), total=len(self.socks_list)):
